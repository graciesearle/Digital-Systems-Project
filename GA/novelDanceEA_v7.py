...
def create_random_pose_gene():
    ...
    hip_x = random.uniform(0.4, 0.6)
    hip_y = random.uniform(0.3, 0.5) 

    # 2. Angles: bias towards upright pose
    # Torso (Up)
    tor_ang = random.uniform(math.pi/2 - 0.5, math.pi/2 + 0.5)
    # Head (Up)
    head_ang = random.uniform(math.pi/2 - 0.5, math.pi/2 + 0.5)

    # Arms (Down-ish or Out)
    l_sh_ang = random.uniform(math.pi, 2 * math.pi) # Pointing down/left
    l_el_ang = random.uniform(math.pi, 2 * math.pi)
    r_sh_ang = random.uniform(math.pi, 0)           # Pointing down/right
    if r_sh_ang < 0: r_sh_ang += 2 * math.pi
    r_el_ang = random.uniform(math.pi, 0)

    # Legs (Down)
    # 1.5 pi is straight down (270 degrees)
    l_hip_ang = random.uniform(3 * math.pi / 2 - 0.5, 3 * math.pi / 2 + 0.5)
    l_knee_ang = random.uniform(3 * math.pi / 2 - 0.5, 3 * math.pi / 2 + 0.5)
    r_hip_ang = random.uniform(3 * math.pi / 2 - 0.5, 3 * math.pi / 2 + 0.5)
    r_knee_ang = random.uniform(3 * math.pi / 2 - 0.5, 3 * math.pi / 2 + 0.5)

    return [hip_x, hip_y, tor_ang, head_ang, 
            l_sh_ang, l_el_ang, 
            r_sh_ang, r_el_ang, 
            l_hip_ang, l_knee_ang, 
            r_hip_ang, r_knee_ang]
...
def generate_standard_poses():
    # Pose 1: Standing Neutral (Hands down)
    REFERENCE_POSES.append({
        "Hips": (0.5, 0.4),
        "L_Hand": (0.55, 0.35), "R_Hand": (0.45, 0.35), # Hands by sides
        "L_Foot": (0.55, 0.0),  "R_Foot": (0.45, 0.0)
    })
    # Pose 2: T-Pose (Hands out)
    REFERENCE_POSES.append({
        "Hips": (0.5, 0.4),
        "L_Hand": (0.75, 0.6), "R_Hand": (0.25, 0.6), # Hands out
        "L_Foot": (0.55, 0.0), "R_Foot": (0.45, 0.0)
    })
    # Pose 3: Hands Up (Cheering)
    REFERENCE_POSES.append({
        "Hips": (0.5, 0.4),
        "L_Hand": (0.6, 0.8), "R_Hand": (0.4, 0.8), # Hands up
        "L_Foot": (0.55, 0.0), "R_Foot": (0.45, 0.0)
    })
...
def calculate_novelty(pose_coords):
    ...
    # We compare specific key joints to see if the 'vibe' is different
    check_joints = ["L_Hand", "R_Hand", "L_Foot", "R_Foot"]
    
    for ref_pose in REFERENCE_POSES:
        dist_sum = 0
        for joint in check_joints:
            p1 = pose_coords[joint]
            p2 = ref_pose[joint]
            # Euclidean distance
            d = math.sqrt((p1[0] - p2[0])**2 + (p1[1] - p2[1])**2)
            dist_sum += d
        
        # We want the distance to the CLOSEST match
...
def calculate_fitness(dance_genome):
    ...
    MAX_ROTATION_PER_FRAME = 0.5 # 0.5 radians is approx 30 degrees

    for i in range(GENOME_LENGTH):
        pose = dance_poses[i]

        # --- NOVELTY BONUS ---
        # Calculate how different this pose is compared to standard poses
        novelty_score = calculate_novelty(pose)

        # Cap the novelty:
        if novelty_score > 1.5:
            total_score -= 50  # Penalise outliers
        else:
            total_score += novelty_score * 20  # Reward novelty

    for i in range(GENOME_LENGTH - 1):
        pose = dance_poses[i]

        # --- A. Elbow Biomechanics Check ---
        # Need the raw angles from the genome to check this easily
        # Genome index: [0,1-Hips, 2=Tor, 3=Head, 4=L_Sh, 5=L_El, 6=R_Sh, 7=R_El, ...]
        current_genes = dance_genome[i]

        # Left Arm
        l_sh_ang = current_genes[4]
        l_el_ang = current_genes[5]
        # Calculate relative angle at elbow. 0 means arm is straight
        l_rel_elbow = (l_el_ang - l_sh_ang + math.pi) % (2 * math.pi) - math.pi
        # If relative angle is less than 0 (bent backwards), penalize
        if abs(l_rel_elbow) > 1.8:
            total_score -= 50

        # Right Arm
        r_sh_ang = current_genes[6]
        r_el_ang = current_genes[7]
        r_rel_elbow = (r_el_ang - r_sh_ang + math.pi) % (2 * math.pi) - math.pi
        if abs(r_rel_elbow) > 1.8:
            total_score -= 50
        ...
        if consecutive_air_frames > MAX_AIR_TIME:
            total_score -= 50  # Penalty for being in air too long (flying)

        # Check if feet are under the floor (clipping)
        # Punish based on how far below ground level they are
        if l_foot_y < GROUND_LEVEL:
            total_score -= abs(l_foot_y - GROUND_LEVEL) * 500

        if r_foot_y < GROUND_LEVEL:
            total_score -= abs(r_foot_y - GROUND_LEVEL) * 500

        # Upright Posture Bonus
        if pose["Head"][1] > pose["Hips"][1]:
            total_score += 2

        # --- C. Movement & Velocity Check ---
