...
def analyze_music(audio_path):
    ...
    # --- FIX START ---
    # Newer librosa returns tempo as an array (e.g., [110.4]). 
    # We must extract the float value.
    if isinstance(tempo, np.ndarray):
        tempo = tempo.item()
    # --- FIX END ---
    
    # 2. Convert frames to time (seconds)
...
# Usage
# beat_times, audio_data, sample_rate = analyze_music("my_song.mp3")
...
def calculate_fitness(dance_genome, debug=False):
    ...
    # --- D. MUSICALITY (Rhythm Sync) ---
    for i in range(GENOME_LENGTH - 1):
        pose_A = dance_poses[i]
        pose_B = dance_poses[i+1]
        
        # Calculate total body movement energy
        frame_energy = 0
        for joint in ["L_Hand", "R_Hand", "L_Foot", "R_Foot"]:
            p1, p2 = pose_A[joint], pose_B[joint]
            dist = math.sqrt((p1[0]-p2[0])**2 + (p1[1]-p2[1])**2 + (p1[2]-p2[2])**2)
            frame_energy += dist
            
        # THE REWARD:
        # If the music is fast/intense, we want high energy.
        # Ideally, you'd extract "loudness" from librosa for each beat.
        # For now, we simply require minimal movement per beat to prevent "sleeping"
        
        if frame_energy < 0.1: 
            total_score -= 10 # Boring! Missed the beat!
        elif frame_energy > 0.5:
            total_score += 20 # Good! Hit the beat hard!
        else:
            total_score += frame_energy * 10 # Linear reward
